...
WORKDIR /app
# FIX: Cargo.lock is not generated by the scaffolder — copy Cargo.toml only
COPY Cargo.toml ./
...
function generateDockerCompose(projectName, database) {
  const services = {
    app: {
      ...
      // FIX: only reference the "db" service when it actually exists
      // (redis service is named "redis", sqlite has no service at all)
      depends_on: ['postgresql', 'mongodb', 'mysql'].includes(database) ? ['db'] : []
    }
  };
  ...
  for (const [name, config] of Object.entries(services)) {
    ...
    yaml += `  ${name}:\n`;
    // FIX: emit image only for prebuilt service images (db/redis),
    // build only for the app service — never both, never swapped
    if (config.image) {
